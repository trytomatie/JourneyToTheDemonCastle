...
void ApplyFoliageWind_float(in float3 wPos, in float maxStrength, in float mask, in float leafFlutter, in float globalMotion, in float swinging, in float freqMult, in float3 positionOS,in float speedScale, out float3 offset)
{
	float speed = WindSpeed() + speedScale * 0.005f;
