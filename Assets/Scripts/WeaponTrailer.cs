...
public class WeaponTrailer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _vertTime = new float[_vertices.Length];
    ...
    void LateUpdate()
    {
        ...
        //_vertices[_frameCount + 3] = _previousTipPosition;
        //_vertices[_frameCount + 4] = _base.transform.position;
        _vertices[_frameCount + 3] = _previousBasePosition;
        for (int i = 0; i < 6; i++)
        {
            _vertTime[_frameCount + i] = Time.time;
        }
        ...
        _triangles[_frameCount + 3] = _frameCount;
        _triangles[_frameCount + 4] = _frameCount + 2;
        _triangles[_frameCount + 5] = _frameCount + 3;
        ...
        //{
        //    colors[i] = new Color((Time.time - _vertTime[i]), 0, 0,1);
        //}
        //_mesh.colors = colors;
        Vector2[] uvs = new Vector2[_vertices.Length];
        for(int i = 0; i < _vertices.Length; i++)
        {
            uvs[i] = new Vector2((Time.time - _vertTime[i]), i % 2);
        }
        _mesh.uv = uvs;
